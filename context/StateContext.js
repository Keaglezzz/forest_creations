...
export const StateContext = ({ children }) => {
  ...
  const onRemove = (product) => {
    ...
    setTotalPrice(
      (prevTotalPrice) =>
        prevTotalPrice -
        foundProduct.selectedVolumePrice * foundProduct.quantity
    );
    setTotalQuantities(
  ...
  const onAdd = (product, quantity) => {
    const productWithVolumeId = product.custom
      ? `custom-${product._id}-${Date.now()}`
      : `${product._id}-${product.price}`;

    const existingCartItem = cartItems.find(
      (cartItem) => cartItem._id === productWithVolumeId
    );
      ...
      if (item._id === id) {
        ...
        if (value === "inc") {
          ...
          if (updatedItem.selectedVolume) {
            setTotalPrice(
              (prevTotalPrice) =>
                prevTotalPrice - updatedItem.selectedVolume.price
            );
          } else {
            setTotalPrice(
              (prevTotalPrice) => prevTotalPrice - updatedItem.price
            );
          }
          setTotalQuantities((prevTotalQuantities) => prevTotalQuantities - 1);
        }
